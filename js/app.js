(() => {
    ...
    function initSliders() {
        if (document.querySelector(".navigation__swiper")) new core(".navigation__swiper", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: "auto",
        ...
        if (document.querySelector(".banner__slider")) new core(".banner__slider.swiper", {
            modules: [ Autoplay ],
            observer: true,
            observeParents: true,
            slidesPerView: 4,
        ...
        if (document.querySelector(".reviews__slider")) new core(".reviews__slider.swiper", {
    ...
    da.init();
    function bodyFade(status) {
        return true === status ? document.querySelector(".fade").classList.add("active") : document.querySelector(".fade").classList.remove("active");
    }
    function lockBody(status) {
        const BODY = document.body;
        return true === status ? BODY.classList.add("lock") : BODY.classList.remove("lock");
    }
    document.addEventListener("click", (event => {
        const LANGUAGE = {
            menu: document.querySelector(".language"),
            select: document.querySelector(".language__select"),
            button: document.querySelector(".language button"),
            title: document.querySelector(".language .item__title")
        };
        function checkStatus(element, activeClass) {
            return element.classList.contains(activeClass ? activeClass : "active");
        }
        if (LANGUAGE.menu.contains(event.target)) {
            LANGUAGE.menu.classList.toggle("active");
            if (event.target.dataset.lang) LANGUAGE.title.textContent = event.target.dataset.lang.toUpperCase();
        } else if (checkStatus(LANGUAGE.menu)) LANGUAGE.menu.classList.remove("active");
        const BURGER = {
            menu: document.querySelector(".menu"),
            button: document.querySelector(".menu__button"),
            items: document.querySelector(".burger-nav")
        };
        if (BURGER.button === event.target || BURGER.button.contains(event.target)) BURGER.menu.classList.toggle("active"); else if (BURGER.menu.contains(event.target)) ; else if (checkStatus(BURGER.menu)) BURGER.menu.classList.remove("active");
        const MODAL_SEARCH = {
            bar: document.querySelector(".modal-search"),
            button: document.querySelector(".search__button")
        };
        if (MODAL_SEARCH.button === event.target || MODAL_SEARCH.button.contains(event.target) || MODAL_SEARCH.bar.contains(event.target)) {
            MODAL_SEARCH.bar.classList.add("active");
            bodyFade(true);
            lockBody(true);
        } else if (checkStatus(MODAL_SEARCH.bar)) {
            MODAL_SEARCH.bar.classList.remove("active");
            bodyFade(false);
            lockBody(false);
        }
        const MAP = {
            button: document.querySelector(".legend__adress button"),
            modal: document.querySelector(".modal-map"),
            close: document.querySelector(".modal-map__close")
        };
        if (MAP.button === event.target || MAP.button.contains(event.target)) {
            MAP.modal.classList.add("active");
            bodyFade(true);
            lockBody(true);
        } else if (checkStatus(MAP.modal) && (!MAP.modal.contains(event.target) || MAP.close === event.target || MAP.close.contains(event.target))) {
            MAP.modal.classList.remove("active");
            bodyFade(false);
            lockBody(false);
        }
        const CONTACTS = {
            button: document.querySelector(".phones__button"),
            modal: document.querySelector(".phones"),
            close: document.querySelector(".phones__close")
        };
        if (CONTACTS.button === event.target || CONTACTS.button.contains(event.target)) CONTACTS.modal.classList.toggle("active"); else if (checkStatus(CONTACTS.modal) && (!CONTACTS.modal.contains(event.target) || CONTACTS.close === event.target || CONTACTS.close.contains(event.target))) CONTACTS.modal.classList.remove("active");
        const REVIEWS = {
            more: document.querySelectorAll(".content__more")
        };
        if (event.target.closest(".slide.text-overflow") && [ ...REVIEWS.more ].includes(event.target)) {
            const close = event.target.closest(".slide.text-overflow");
            close.classList.toggle("show");
            if (checkStatus(close, "show")) {
                console.log(close.querySelector(".content__text"));
                close.querySelector(".content__text").style.height = `${close.querySelector(".content__text").clientHeight}px`;
            }
        } else document.querySelectorAll(".slide.text-overflow.show").forEach((x => x.classList.remove("show")));
    }));
    isWebp();
})();